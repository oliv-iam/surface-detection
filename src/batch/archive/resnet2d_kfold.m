...
function kfold(k, data, name)
	...
	for i = 1:5 
        ...
        parfor (j = 1:k)
			% train model
			net = resnet2d(data_train{j});
		
			% check accuracy on test set
			tmp = neteval(net, data_train{j}, "image", ...
				"logs/scratch/resnet2d_trainpreds_" + name + ".txt", j==0);
			kacc(j) = neteval(net, data_test{j}, "image", ...
				"logs/scratch/resnet2d_testpreds_" + name + ".txt", j==0);
        end
        toc

        % write to log file
        f = fopen("logs/scratch/resnet2d_acc_" + name + ".txt", "a+");
        fprintf(f, "%f,", kacc);
        fprintf(f, "\n");
